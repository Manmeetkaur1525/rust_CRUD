    ...
    match stream.read(&mut buffer) {
        Ok(size) => {
            ...
            let (status_line , content) = match &*request {
                ...
                r if request.starts_with("GET /users/all") => handle_get_all_requests(r),
...
fn handle_get_all_requests(request: &str) -> (String , String) {
...
fn handle_put_request(request: &str) -> (String, String) {
    ...
    )
    {
        (Ok(id), Ok(user),Ok(mut client)) => {
            client
            .execute("UPDATE users SET name =$1, email = $2 WHERE id = $3",
        &[&user.name , &user.email, &id]).unwrap();
        (OK_RESPONSE.to_string(), OK_RESPONSE.to_string())
        }
        _ => (INTERNAL_SERVER_ERROR.to_string(),"ERROR".to_string()),
    }
}
